		...
		if (handler == nullptr) {
			...
			if (soft_logs.fetch_add(1, std::memory_order_relaxed) < 32) {
				LOGF_COLOR(Log::Color::Yellow,
				           ...
				           packet_header, total_dw - remaining_dw);
			}
			// Prefer the encoded packet length when present; otherwise skip one header dword.
			auto packet_len = KYTY_PM4_LEN(packet_header);
			if (packet_len == 0 || packet_len > remaining_dw) {
				packet_len = 1;
			}
			cursor.next_packet += packet_len;
			cursor.remaining_dw -= packet_len;
			execution.m_made_progress = true;
			continue;
		}
