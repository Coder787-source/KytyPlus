...
3. If a newer report contradicts an older row for the same title, the newer report wins — link
   both issues in the Reports column so history isn't lost.
4. Rows are sorted alphabetically by title. Keep one row per title; add new report links to the
   Reports column instead of duplicating rows.
